...
public class FullMapCameraPosition : MonoBehaviour
{
    ...
    static float orthographicSize = 700;
    // Threshold for maximum x-width before resizing full map
    static float xThresh = 1000;
    ...
    void ResizeFullMap()
    {
        ...
        if (Math.Abs(xDiff) > xThresh || Math.Abs(zDiff) > zThresh)
        {
            ...
            else
            {
                ...
                newOrthographicSize = orthographicSize * zPercentage/2;
            }
